...
const CompanyDetail: React.FC = () => {
    ...
    const { loading, err, company, wars, members, leaderboards } = useCompanyDetails(companyName);

    if (loading) return (<span className="text-white" ><Loading /></span>);
    if (err || !company) return (<span className="text-white" ><NotFound /></span>);

    const content = [
        <CompanyStatOverview companyName={companyName} wars={wars} leaderboard={leaderboards} />,
        <CompanyWarHistory companyName={companyName} wars={wars.slice(0, 5)} />,
